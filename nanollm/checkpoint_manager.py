...
from nanollm.common import get_base_dir, setup_default_logging
from nanollm.models import get_model_entry, map_hf_config, map_hf_state_dict
...
def load_checkpoint(
    checkpoint_dir, step, device, load_optimizer=False, rank=0,
    ...
    if load_optimizer:
        ...
        optimizer_data = torch.load(
            optimizer_path,
            map_location=device if optimizer_device is None else optimizer_device,
        )
    # Load the metadata
    meta_path = os.path.join(checkpoint_dir, f"meta_{step:06d}.json")
...
def build_model(checkpoint_dir, step, device, phase):
    ...
    architectures = model_config_kwargs.get("architectures")
    if not architectures:
        raise ValueError("No architectures found in model config. Cannot determine model type.")
...
# -----------------------------------------------------------------------------
# convenience functions that take into account nanollm's directory structure
...
def load_pretrained_hf(pretrained_dir, device, phase="eval", **kwargs):
    ...
    state_dict = map_hf_state_dict(hf_state_dict, entry["state_dict_mapper"])
    ...
    # from multimodal checkpoints that are not part of the text-only model).
