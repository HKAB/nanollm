"""
Common utilities for nanollm. Copy directly from NANOLLM.
"""
...
# Override with NANOLLM_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("NANOLLM_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via NANOLLM_DTYPE={env}"
    if torch.cuda.is_available():
        ...
        # Users can still force fp16 via NANOLLM_DTYPE=float16 if they know what they're doing.
...
def get_base_dir():
    # co-locate nanollm intermediates with other cached data in ~/.cache (by default)
    if os.environ.get("NANOLLM_CACHE_DIR"):
        nanollm_dir = os.environ.get("NANOLLM_CACHE_DIR")
    else:
...
# and PR: https://github.com/karpathy/nanollm/pull/147
